...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            tick align=outside,
            ytick={0, 0.5e7, 1e7, 1.5e7},
            yticklabels={$0$, $0.5 \cdot 10^{7}$, $1 \cdot 10^{7}$, $1.5 \cdot 10^{7}$},
            scaled y ticks=false,
            symbolic x coords={0,4,8,12,16,20,24,28,32},
            grid=minor,
            ylabel={Number of locks and unlocks per second},
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{\textbf{Workload 1}: Lock and unlock operations per second}
    \label{fig:workload1}
\end{figure}
...
\begin{figure}[H]
    \begin{tikzpicture}
        ...
        \begin{axis}[
            ...
            tick align=outside,
            grid=minor,
            ytick={0, 0.2e6, 0.4e6, 0.6e6, 0.8e6, 1e6, 1.2e6},
            yticklabels={$0$,$0.2 \cdot 10^{6}$, $0.4 \cdot 10^{6}$, $0.6 \cdot 10^{6}$, $0.8 \cdot 10^{6}$, $1.0 \cdot 10^{6}$, $1.2 \cdot 10^{6}$},
            scaled y ticks=false,
            ylabel={Number of locks and unlock per second},
            xlabel={Number of worker threads},
            mark options={solid},
            legend style={at={(0.5,-0.25)}, anchor=north,legend columns=-1},
            ]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{\textbf{Workload 2}: Lock and unlock operations per second}
    \label{fig:workload2}
\end{figure}
...
This range lock is lightweight and efficient, especially for managing small to moderate numbers of logical blocks in memory.
...
We evaluated the throughput of four competitors as the number of worker threads increased, as depicted in Figure \ref{fig:txn_comparison}. 
...
\begin{figure}[h]
    ...
    \caption{Cycles, instructions, and L1-misses}
