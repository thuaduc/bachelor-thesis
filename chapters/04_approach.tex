...
\begin{figure}[H]
    ...
    % Arrows and Labels
    \path[->] 
        (worker1) edge[draw=blue] node[left,yshift=-1cm] {tryLock(0,512) -- \ref{subsec:tryLock}} (crl)
        (crl) edge[draw=blue,bend left=10] node[left] {success} (success1)
        (success1) edge[draw=darkblue,bend left=10] node[right] {releaseLock(0,512) -- \ref{subsec:releaseLock}} (crl)
